...
rm(list=ls())
getwd()
setwd("~/GitHub/QB2017_Gibson/Week4-Spatial")
...
> ***Answer 1***:  Simba is for calculating similarity measures, rgdal is a geospatial abstraction (?) library.
...
> ***Answer 4a***:  A datum is the model of Earth's shape (of which there are many) and a projection is the way which we take coordinates on a sphere and "project" them onto the 2D map.
...
> ***Answer 5***: Spatial autocorrelation is the degree to which sites that are close to one another in space have similar trait values. !!!!!
...
> ***Answer 6***: Distance decay patterns (or graphs) describe how species composition similarity (or dissimilarity) between sites is related to the geographic distance between sites. If this pattern shows a negative relationship, that would be evidence for spatial autocorrelation (sites closer together are more similar to one another).  !!! 
